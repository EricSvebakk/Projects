...
class App extends Component {
  ...
  toggleSubjectRecursive = (nodeList) => {
    ...
    for (const node of nodeList) {
      if (node.msubs != null) {
        let rec = data.nodes.filter((n) => node.msubs.includes(n.id));
        result = [...result, ...this.toggleSubjectRecursive(rec)];
      }

      if (node.rsubs != null) {
        let rec = data.nodes.filter((n) => node.rsubs.includes(n.id));
        result = [...result, ...this.toggleSubjectRecursive(rec)];
      }
    }
          ...
          <section className="left">
            <div>
              ...
              <div style={{ display: "inline-block" }}>
                <button className="subject-button" onClick={this.clearSubjects}>
                  clear
                </button>
